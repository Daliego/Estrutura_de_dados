...
void inserirOrdenado(Item item, Lista *L) {
...
bool removeItem(Item item, Lista *L) {
    while (*L != NULL && (*L)->item < item) {
        L = &(*L);
        if ((*L)->item == item) {
            Lista listaAuxiliar = *L;
            *L = listaAuxiliar->prox;
            free(listaAuxiliar);
            return 1;
        }
    }
...
void showItemsFromEnd(Lista lista) {
    if (lista != NULL) {
        showItemsFromEnd(lista->prox);
    }
...
bool searchItens(Item x, Lista lista) {
    while (lista != NULL) {
        if (lista->item == x) {
            return 1;
        }
